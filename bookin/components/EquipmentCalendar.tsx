    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <button
          ...
          className="font-mono text-sm px-3 py-1.5 border border-line hover:border-ink focus-ring"
        >
          ← Prev
        </button>
        <div className="text-center">
          <p className="font-display font-600">{format(day, "EEEE, MMM d")}</p>
          {isSameDay(day, new Date()) && (
            <p className="text-xs font-mono text-teal">Today</p>
          )}
        </div>
        <button
          onClick={() => setDay((d) => addDays(d, 1))}
      ...
      <div className="flex items-center gap-4 mb-4 text-xs font-mono text-ink-soft">
      ...
      {loading ? (
        <p className="font-mono text-sm text-ink-soft py-8 text-center">Loading calendar…</p>
      ) : (
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-4 px-4 py-2.5 text-left transition-colors focus-ring ${
                  isClickable ? "hover:bg-amber/10 cursor-pointer" : "cursor-default"
                } ${status === "past" ? "opacity-40" : ""}`}
              >
                <span className="font-mono text-xs text-ink-soft w-16 shrink-0">
                  {format(slot, "h:mm a")}
                </span>
                {status === "booked" && (
      ...
      {!userId && (
        <p className="mt-4 text-xs font-mono text-ink-soft">Sign in to book a slot.</p>
      )}
